...
public class RedirectForward01Controller {
	...
	public String re3(HttpServletRequest request) {
		...
		return "re/re2"; // 요청 > /re3 > 화면(view) re2로 보여줌
		// 페이지 내부의 redirect 처리
	...
	public String re6(HttpServletRequest request) {
		...
		// foward 서버에 바로 여청 경로를 바꿔서 수행
		// 그대러 전달

		return "forawrd:/re/re2/"; // RequestMapping 에 해당하는 경로를 작성
	...
	public String re7(HttpServletRequest request, RedirectAttributes ra) {
		...
		//request.setAttribute("msg", request.getParameter("msg")); // view 데티어 객체전달용
		//redirect 시에 새로운 rdquest 로 요청되기 때문에 redirect 하면 넘어가지 않는다
		
		//return "redirect:re/re2/
		

//		return "redirect:/re/re2?msg="+ request.getParameter("msg"); // /re/re2/ 경로로 redirect
//		ra.addAttribute("msg", request.getParameter("msg")); // 경로 파라미터 추가해서 연결
			//redirect 요청 주소 경로 뒤에 파라미터 추가해서 연결
		//	> 연결된 서버측 메소드에서도 경로에있는 Parameter 확인가능
