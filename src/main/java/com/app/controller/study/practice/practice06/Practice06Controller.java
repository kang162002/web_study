...
import com.app.controller.dto.study.practice.practice06.PersonBmi;

public class Practice06Controller {
	/*
	 * 1. /practice06/ask-bmi 경로로 요청시, 화면에는 이름 / 키 / 몸무게를 입력하는 항목과 확인하기 버튼이 있습니다. 버튼
	 * submit 발생
	 */
	...
	/*
	 * 2. /practice06/result-bmi 경로로 이동하여 요청에 따라서 컨트롤러가 비만도를 계산해서 결과를 보여줍니다.
	 * 
	 * 화면에는 사용자가 입력한 이름 / 키 / 몸무게가 표시되고, 추가로 BMI 가 표시됩니다.
	 */
	...
	public String result_bmi2(@RequestParam String name, @RequestParam String height, @RequestParam String weight,
			Model model) {
	...
	public String result_bmi4(@ModelAttribute PersonBmi personBmi, Model model) {
		...
		// 결과화면 result-bmi 페이지에서 보여줄 값 세팅
		/*
		 * model.addAttribute("name", name); model.addAttribute("height", height);
		 * model.addAttribute("weight", weight); model.addAttribute("bmi", bmi);
		 */
